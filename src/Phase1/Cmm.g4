...
setter					: SET {System.out.println("Setter");} BEGIN NEW_LINE*
							scope_body* END (SEMICOLON | NEW_LINE)+
						| SET {System.out.println("Setter");} NEW_LINE*
							scope_body?
							SEMICOLON* (SEMICOLON | NEW_LINE)+;

getter					: GET {System.out.println("Getter");} BEGIN NEW_LINE*
							scope_body* END
							(SEMICOLON | NEW_LINE)+
						| GET NEW_LINE+ {System.out.println("Getter");}
							scope_body? (SEMICOLON | NEW_LINE)+;
...
body					: (var_dec | statement | expression | NEW_LINE | SEMICOLON)*;
var_dec					: (int_bool_dec | list_dec | struct_ins | fptr_dec)+;

function				: function_dec BEGIN NEW_LINE* body NEW_LINE* END NEW_LINE*
						| function_dec NEW_LINE* (var_dec | statement | expression) (NEW_LINE | SEMICOLON)*;
...
expression              : function_call | display_statement | size_statement | append_statement | return_statement
						...
						| expression X=(DIVIDE | MULTIPLY) expression
							{System.out.println("Operator : " + $X.getText());}
...
statement				: function_call_statement | conditional_statement | loop_statement | return_statement
						| display_statement | size_statement | append_statement ;

conditional_statement	: IF {System.out.println("Conditional : if");} NEW_LINE* condition
							scope_body? NEW_LINE* else_statement?
       					| IF {System.out.println("Conditional : if");} NEW_LINE* condition BEGIN NEW_LINE*
       						scope_body* END
       						NEW_LINE* else_statement?;

condition				: OPEN_PARENTHESES expression CLOSE_PARENTHESES NEW_LINE* | expression NEW_LINE*;

else_statement			: ELSE {System.out.println("Conditional : else");} NEW_LINE*
							scope_body? NEW_LINE*
                        | ELSE {System.out.println("Conditional : else");} NEW_LINE* BEGIN NEW_LINE*
                        	scope_body* NEW_LINE* END;
...
while_statement			: WHILE {System.out.println("Loop : while");} NEW_LINE*
							OPEN_PARENTHESES? expression CLOSE_PARENTHESES? NEW_LINE*
							BEGIN NEW_LINE* (statement | expression | var_dec | NEW_LINE | SEMICOLON)* NEW_LINE* END NEW_LINE*
						| WHILE {System.out.println("Loop : while");} NEW_LINE*
							OPEN_PARENTHESES? expression CLOSE_PARENTHESES? NEW_LINE*
							NEW_LINE* (statement | expression | var_dec | NEW_LINE | SEMICOLON)? NEW_LINE*;
...
list_element			: OPEN_BRACKETS (INTEGER_VALUE | expression | list_element)
							CLOSE_BRACKERTS (SEMICOLON | NEW_LINE)*;
...
int_bool_dec			: (INT | BOOL) (var_dec_name (ASSIGN expression)* COMMA)* var_dec_name (NEW_LINE | SEMICOLON)*
    					| (INT | BOOL) (var_dec_name (ASSIGN expression)* COMMA)* var_dec_name ASSIGN
...
struct_ins				: STRUCT_DECLARATION IDENTIFIER (var_dec_name (ASSIGN expression)* COMMA)*
							var_dec_name (SEMICOLON | NEW_LINE)*
						| STRUCT_DECLARATION IDENTIFIER (var_dec_name (ASSIGN expression)* COMMA)*
							var_dec_name ASSIGN (expression COMMA)* expression (SEMICOLON | NEW_LINE)*;

fptr_dec				: FUNCTIOR_POINTER LESS_THAN ((function_type COMMA)* function_type ) MINUS GRATER_THAN
							function_type GRATER_THAN (var_dec_name (ASSIGN expression)* COMMA)*
							var_dec_name (SEMICOLON | NEW_LINE)*
						| FUNCTIOR_POINTER LESS_THAN ((function_type COMMA)* function_type ) MINUS GRATER_THAN
							function_type GRATER_THAN (var_dec_name (ASSIGN expression)* COMMA)* var_dec_name ASSIGN
							//{System.out.println("Operator : =");}
							(expression COMMA)* expression (SEMICOLON | NEW_LINE)*;
